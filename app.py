# This is the main app that when ran will query the disneyland website and compare the results to the s3 record before sending updates via
...
def get(url: str) -> dict:
    """Get the data from the provided url and return the json data

    Args:
        url (str): The url to get the data from

    Raises:
        RuntimeError: Throws a error if the request doesn't return a 200
...
def get_s3_record() -> dict:
    """Get past data from this application from S3 and return it as a dictionary. This will return an empty dictionary if there is no data in S3.

    Returns:
        dict: a dictionary of the data fetched from s3
    """
...
def save_s3_record(data: dict) -> None:
    """Save new data to S3

    Args:
        data (dict): the updated record to store to the s3 bucket
    """
...
def get_availability_data() -> dict:
    """Queries the disneyland website and returns the availability data as a dictionary

    Returns:
        dict: map of restaurant names to a list of their availability
    """
    # Get all dining data from the websites
    raw_dining_data = get(
        f"https://disneyland.disney.go.com/finder/api/v1/explorer-service/list-ancestor-entities/dlr/80008297;entityType=destination/{TARGET_DATE}/dining"
    )

    # For now what we really want out of this is the mapping of dining locations ids to their names
    ...
    restaurant_map = {
        ...
        if "entityType=restaurant" in restaurant["id"]
    }

    # Now that we have that we can try our filtered request
    raw_availability_data = get(
        "https://disneyland.disney.go.com/finder/api/v1/explorer-service/dining-availability-list/{13019931-306C-4B87-AFAD-050364842981}/dlr/80008297;entityType=destination/2022-08-12/5/?mealPeriod=80000717"
    )
    ...
    for restaurant_id, data in raw_availability_data["availability"].items():
        if data["hasAvailability"] and "singleLocation" in data.keys():
            availablility_map[restaurant_map[restaurant_id]] = data["singleLocation"][
                "offers"
            ]
    ...
    return {name: offers for name, offers in availablility_map.items()}
...
def send_email(subject, body):
    ...
    ses.send_email(
        ...
        Destination={"ToAddresses": [os.environ["EMAIL"]]},
